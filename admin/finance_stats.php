            ...
            <section class="scrollable wrapper">
              <div class="panel b-a">
                ...
                <div class="panel-body">
                  <div class="h3 m-b font-thin">Información de las Finanzas de la Alianza</div>
                  <div class="row">
                    <div class="col-sm-12">    
...
<div class="row">
	...
	for ($i = 1 ; $i <= 12 ; $i++) {
		$days = $days . ',' . $i;
		$sql2 = "select IFNULL(ROUND(sum(amount),0),0) as co from va_finances where date_format(finance_date,'%y')=date_format(now(),'%y') and date_format(finance_date,'%m')=$i and operator_id IN (" . implode(',', array_map('intval', $airlines)) . ")";
	...
	// Execute SQL query
	$total_amount=0;
	$sql = "select sum(amount) as total_amount from va_finances where operator_id IN (" . implode(',', array_map('intval', $airlines)) . ")";
	...
	$sql = "SELECT parameter_id,financial_parameter,sum(vaf.amount) as vaf_amount FROM  va_finances vaf inner join financial_parameters fp on vaf.parameter_id = fp.id where is_cost=1 and operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id,financial_parameter ";
	if (!$result = $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}

	$sql = "SELECT parameter_id,financial_parameter,sum(vaf.amount) as vaf_amount FROM  va_finances vaf inner join financial_parameters fp on vaf.parameter_id = fp.id where is_profit=1  and operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id,financial_parameter";
	if (!$result2= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}

	$sql = "SELECT 'Cargo in regular routes' as des,sum(vaf.amount) as vaf_amount FROM  va_finances vaf  where vaf.parameter_id in (99998)  and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id";
	if (!$result3= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}

	$sql = "SELECT 'PAX in regular routes' as des,sum(vaf.amount) as vaf_amount FROM  va_finances vaf  where vaf.parameter_id in (99999) and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id";
	if (!$result4= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}

	$sql = "SELECT description,vaf.amount as vaf_amount FROM  va_finances vaf  where vaf.parameter_id =0 and report_type='New Aircraft' and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ")";
	if (!$result5= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}	
	$sql = "SELECT 'Pilots Flight Salary' as description,sum(vaf.amount) as vaf_amount FROM  va_finances vaf  where vaf.parameter_id in (99995) and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id";
	if (!$result6= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}	
	$sql = "SELECT 'Aircraft Maintenance' as description,sum(vaf.amount) as vaf_amount FROM  va_finances vaf  where vaf.parameter_id in (99997) and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id";
	if (!$result7= $db->query($sql)) {
		die('There was an error running the query  [' . $db->error . ']');
	}
	$sql = "SELECT 'Aircraft Repair' as description,sum(vaf.amount) as vaf_amount FROM  va_finances vaf  where vaf.parameter_id in (99996)  and vaf.operator_id IN (" . implode(',', array_map('intval', $airlines)) . ") group by parameter_id";
