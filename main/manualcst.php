...
window.location = './index_user.php?page=charter';
	...
	$aeronave = $_POST['plane'];
	
    $queryplane = "select * from fleets f 
	inner join fleettypes ftr on ftr.fleettype_id = f.fleettype_id where f.fleet_id='$aeronave'";
	if (!$resultplane = $db->query($queryplane)) {
		die('There was an error running the query [' . $db->error . ']');
	}
	while ($rowpl = $resultplane->fetch_assoc()) {
		$matricula = $rowpl['registry'];
		$icao = $rowpl['plane_icao'];
		$plane_operator_id = $rowpl['operator_id'];
	}
	
	if($icao_va<>$plane_operator_id)
	{
		?>
...
// Set number of PAX & cargo and get plane details
		$sql70 = "select * from fleets f inner join fleettypes ft on f.fleettype_id = ft.fleettype_id and registry='$registry'";
...
if($percent>=94 && $estado==1) {
	...
	values ('$icao_va','$callsign2','$vid2','$pilot','$airplane','$cruisingspeed','$salida','$llegada','$trasponder','$ruta','$rmk','$etod','$ete','$duration','$distnms','$arrivaltime','$tipes','$requestlevel',now(),1,'$registry','$assigned_pax','$assigned_cargo','0','$code_info_money')";
	...
	$sql1123p = "UPDATE fleets set booked='0', booked_at='0', gvauser_id='0',location='$llegada',hours=hours+$duration,status=status-$flight_wear where registry='$registry'";
		...
		if ($estado < $plane_status_hangar && $estado > 0) {
					$query1 = "insert into hangar (registry,gvauser_id,fleet_id,departure,location,date_in,date_out,reason) values ('$matricula',$pilot,$avion,'$salida','$llegada',CURDATE(),ADDDATE(CURDATE(),$hangar_maintenance_days),'Maintenance')";
					...
					$query1 = "insert into vaprofits (operator_id,value,date,gvauser_id,description) values ('$icao_va',-$hangar_maintenance_value, now(),$pilot ,'Maintenance $matricula')";
					
					if (!$result_sta = $db->query($query1)) {
						die('There was an error running the query [' . $db->error . ']');
					}
					// Cost for the VA for the maintenance
				    $query1 = "insert into va_finances (operator_id,amount,parameter_id,finance_date,gvauser_id,description,report_type,report_id) values ('$icao_va',-$hangar_maintenance_value, '99997', now(),$pilot ,'Aircraft Maintenace $matricula','IVAO', '$code_info_money')";
				    if (!$result_sta = $db->query($query1)) {
					  die('There was an error running the query [' . $db->error . ']');
				    }
				}
